...
const request: FiveOneEat.RegisterCustomerRequest = {
    ...
};
```
...
```typescript
import { AuthenticationClient } from '@51eat/sdk/authentication';
...
```typescript
const { data, rawResponse } = await client.authentication.registerCustomer(...).withRawResponse();
